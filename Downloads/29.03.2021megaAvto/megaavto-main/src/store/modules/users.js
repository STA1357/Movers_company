...
export default {
  ...
  mutations: {
    ...
    async adminUser(state, data) {
      state.adminPageUser = await data;
    },
    auth_error(state) {
      state.loading = false;
    },
  ...
  actions: {
    async login({ commit }, body) {
      try {
        commit("auth_request");
        const { data } = await axios.post("http://avto-obmen.net/api/login", {
          email: body.email,
    ...
    async updateAutoRiaId({ commit }, body) {
      try {
        commit("auth_request");
        const { data } = await axios.put(
          "http://localhost:5002/api/updateAutoRiaId",
          {
            autoRiaId: body.autoRiaId,
          },
          {
            headers: {
              Authorization: `Bearer ${
  ...
  getters: {
    ...
    getAutoRiaId: (state) => state.user.autoRiaId,
  },
};
